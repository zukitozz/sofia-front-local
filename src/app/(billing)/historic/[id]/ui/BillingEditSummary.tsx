...
export const BillingEditSummary = ({ billing }: Props) => {
        ...
        <>
                ...
                <span className="text-right font-bold col-span-1">Importe</span>
                {
                    billing.items?.map( (item, index) => (
                        <div key={item.codigo_producto} className="col-span-3 grid grid-cols-3 py-1">
                            ...
                            <span className="text-right">{item.cantidad_venta}</span>
            ...
            <div className="grid grid-cols-2">
                ...
                <span className="text-right">
                    {currencyFormat(billing.gravadas)}
                </span>
                <span>IGV (18%)</span>
                <span className="text-right">{currencyFormat(billing.igv)}</span>
            </div>
            <div className="w-full h-0.5 rounded bg-gray-200 my-3" />
            <div className="grid grid-cols-2">
                <span className="mt-5 text-2xl">Total:</span>
                <span className="mt-5 text-2xl text-right">
                    {currencyFormat(billing.total)}
                </span>
            </div>
        </>
    );
